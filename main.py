import requests, os, psutil, sys, jwt, pickle, json, binascii, time, urllib3, base64, re, socket, threading, ssl, asyncio, random
...
# Fix equie_emote (xHeaders.py has bytes.fromhex with spaces bug)
...
# === LOAD GUEST ACCOUNTS (sb.json) ===
SB_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'sb.json')
GUEST_ACCOUNTS = {}
try:
    with open(SB_FILE, 'r') as f:
        GUEST_ACCOUNTS = json.load(f)
    print(f"[SB] Loaded {len(GUEST_ACCOUNTS)} guest accounts")
except Exception as e:
    print(f"[SB] Could not load sb.json: {e}")

# Track which guest accounts are in use (team_code -> uid)
lobby_bots = {}

# === IN-MEMORY CONFIG (Firebase-এর বদলে) ===
TASKS_CONFIG = [
    {"id": "task1", "name": "JOIN TELEGRAM CHANNEL", "link": "https://t.me/axemoteserver"},
    {"id": "task2", "name": "JOIN 2d TELEGRAM", "link": "https://t.me/axromjanyt"},{"id": "task3", "name": "SUBSCRIBE YOUTUBE", "link": "https://www.youtube.com/@axromjan"},
]
...
# === GLOBAL EVENT LOOP (একটাই loop সব bot এর জন্য) ===
_global_loop = None
...
_loop_thread = Thread(target=start_global_loop, daemon=True)
_loop_thread.start()
time.sleep(0.3)
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, data=data, timeout=aiohttp.ClientTimeout(total=15)) as response:
                ...
                if not open_id or not access_token:
                    print(f"[GeNeRaTeAccEss] Missing fields: {d}")
                    return None, None
...
async def TcPOnLine_user(ip, port, key, iv, AutHToKen, bot_data, reconnect_delay=1.0):
    while True:
        try:
            ...
            await writer.drain()
            # Send FS packet to stay in lobby
            try:
...
# === BOT STARTUP ===
async def StartUserBot(uid_str, password):
    bot_data = {
        'uid': uid_str,
        'online_writer': None,
        'whisper_writer': None,
    ...
    # ── Join এর আগেই bundle packet pre-build করে cache রাখো ──
    try:
...
# === AUTO BUNDLE IDS (random bundle before lobby join) ===
AUTO_BUNDLE_IDS = {
    "rampage":     "914000002",
    "cannibal":    "914000003",
    "devil":       "914038001",
...
# === BUNDLE CACHE REFRESH (background এ পরের join এর জন্য pre-build) ===
async def _refresh_bundle_cache(bot_data):
    """Join এর পরে background এ পরবর্তী bundle pre-build করে রাখো"""
    key = bot_data.get('key')
    iv = bot_data.get('iv')
    region = bot_data.get('region', 'ind')
    try:
        # আগের bundle থেকে আলাদা bundle বেছে নাও
...
# === AUTO BUNDLE CHANGER (প্রতি ১৫ সেকেন্ডে bundle change) ===
async def auto_bundle_changer(bot_data):
    """প্রতি ১৫ সেকেন্ডে random bundle পরায়"""
    key = bot_data.get('key')
    iv = bot_data.get('iv')
    region = bot_data.get('region', 'ind')
    used_bundles = []
...
# === AUTO LEAVE SQUAD AFTER 20 MINUTES ===
async def auto_leave_after_20min(bot_data):
    """Squad join এর ২০ মিনিট পর bot স্বয়ংক্রিয়ভাবে lobby leave করে solo হয়ে যায়"""
    uid = bot_data.get('uid', '?')
    try:
        await asyncio.sleep(1200)  # ২০ মিনিট = ১২০০ সেকেন্ড
        online_writer = bot_data.get('online_writer')
        key = bot_data.get('key')
        iv = bot_data.get('iv')
        ...
        if online_writer and not online_writer.is_closing() and key and iv:
            try:
                if team_code:
                    ...
                    await online_writer.drain()
                    print(f"[AutoLeave] Bot uid={uid} exit packet sent → tc={team_code}")
                # FS (solo lobby) packet পাঠাও যাতে bot solo থাকে
                fs_pkt = await FS(key, iv)
                online_writer.write(fs_pkt)
                await online_writer.drain()
                print(f"[AutoLeave] Bot uid={uid} FS solo packet sent")
            except Exception as e:
                print(f"[AutoLeave] Packet error for uid={uid}: {e}")

        # Squad state reset করো — bot এখন solo
...
async def perform_emote_for_user(bot_data, team_code, uids, emote_id):
    ...
    # ── Join packet build করো (bundle ইতিমধ্যে cache এ আছে) ──
    ...
    print(f"[AutoLeave] 20min timer started for uid={bot_data.get('uid')} tc={team_code}")

    # ── Step 2: Cached bundle তৎক্ষণাৎ পাঠাও (কোনো computation নেই) ──
    ...
    if cached_pkt:
        ...
        # cache miss হলে real-time build করো
        try:
            _bn = random.choice(list(AUTO_BUNDLE_IDS.keys()))
            ...
            if _pkt:
                ...
                await online_writer.drain()
                print(f"[Emote] Bundle '{_bn}' sent (realtime fallback)")
        except Exception as _fe:
            print(f"[Emote] Bundle fallback error: {_fe}")

    # ── Step 3: Background এ পরের join এর জন্য নতুন bundle pre-build ──
    ...
    for uid_str in uids:
        print(f"[Emote] Sending emote={emote_id} to uid={uid_str} (bot_uid={bot_uid})")
    ...
    # ── আগের auto changer বন্ধ করে নতুন শুরু করো ──
    old_task = bot_data.get('auto_bundle_task')
    if old_task and not old_task.done():
        old_task.cancel()
    task = asyncio.get_event_loop().create_task(auto_bundle_changer(bot_data))
    bot_data['auto_bundle_task'] = task
    print(f"[AutoBundle] Started — changes every 15s for uid={bot_data.get('uid')}")
...
def submit_credentials():
    ...
    global _global_loop
    future = asyncio.run_coroutine_threadsafe(StartUserBot(uid, password), _global_loop)
    try:
...
@app.route('/api/random-submit', methods=['POST'])
def random_submit():
    data = request.get_json(force=True)
    team_code = str(data.get('team_code', '')).strip()

    if not team_code:
        return jsonify({"status": "error", "message": "Team Code দিন!"})

    # Check if this lobby already has a bot
    if team_code in lobby_bots:
        existing_uid = lobby_bots[team_code]
        if existing_uid in user_bots and user_bots[existing_uid].get('status') == 'online':
            return jsonify({"status": "already_online", "message": "এই লবিতে আগেই একটি বট আছে!", "uid": existing_uid})

    # Pick an available guest account (not already running as a bot)
    active_uids = set(user_bots.keys())
    available_uid = None
    available_password = None
    for uid_str, password in GUEST_ACCOUNTS.items():
        if uid_str not in active_uids:
            available_uid = uid_str
            available_password = password
            break

    if not available_uid:
        return jsonify({"status": "error", "message": "সব Random Account ব্যবহৃত হয়ে গেছে! পরে চেষ্টা করুন।"})

    global _global_loop
    future = asyncio.run_coroutine_threadsafe(StartUserBot(available_uid, available_password), _global_loop)
    try:
        bot_data, msg = future.result(timeout=35)
        if bot_data.get('status') == 'online':
            user_bots[available_uid] = bot_data
            lobby_bots[team_code] = available_uid
            return jsonify({"status": "success", "message": "Random Bot চালু হয়েছে!", "uid": available_uid})
        else:
            return jsonify({"status": "error", "message": msg})
    except Exception as e:
        print(f"[random-submit] Exception: {e}")
        return jsonify({"status": "error", "message": f"সংযোগ ব্যর্থ: {str(e)[:100]}"})

@app.route('/api/random-auto', methods=['POST'])
def random_auto():
    # Step 1: প্রথমে নতুন (unused) account খোঁজো
    active_uids = set(user_bots.keys())
    available_uid = None
    available_password = None
    for uid_str, password in GUEST_ACCOUNTS.items():
    ...
    # Step 2: সব account চালু থাকলে — solo bot খোঁজো (in_squad=False)
    if not available_uid:
        solo_uid = None
        for uid_str, bd in user_bots.items():
            if bd.get('status') == 'online' and not bd.get('in_squad', False):
                solo_uid = uid_str
                break
        if solo_uid:
            print(f"[random-auto] Reusing solo bot uid={solo_uid}")
            return jsonify({"status": "already_online", "message": "Solo Bot পুনরায় ব্যবহার হচ্ছে!", "uid": solo_uid})

    # Step 3: কোনো solo bot না পেলে — সবচেয়ে পুরনো squad bot নাও (fallback)
    if not available_uid:
        oldest_uid = None
        oldest_time = float('inf')
        for uid_str, bd in user_bots.items():
            if bd.get('status') == 'online':
                join_t = bd.get('squad_join_time') or 0
                if join_t < oldest_time:
                    oldest_time = join_t
                    oldest_uid = uid_str
        if oldest_uid:
            print(f"[random-auto] Fallback: using oldest bot uid={oldest_uid}")
            return jsonify({"status": "already_online", "message": "Bot পুনরায় ব্যবহার হচ্ছে!", "uid": oldest_uid})
        return jsonify({"status": "error", "message": "কোনো Bot পাওয়া যায়নি, পরে চেষ্টা করুন।"})

    # Step 4: নতুন bot শুরু করো
    global _global_loop
    future = asyncio.run_coroutine_threadsafe(StartUserBot(available_uid, available_password), _global_loop)
    try:
        bot_data, msg = future.result(timeout=35)
        if bot_data.get('status') == 'online':
            ...
            # এই account কাজ না করলে next try করো
            next_uid = None
            next_pw = None
            for uid_str, password in GUEST_ACCOUNTS.items():
                if uid_str not in user_bots and uid_str != available_uid:
                    next_uid = uid_str
                    next_pw = password
                    break
            if next_uid:
                future2 = asyncio.run_coroutine_threadsafe(StartUserBot(next_uid, next_pw), _global_loop)
                try:
                    bd2, msg2 = future2.result(timeout=35)
                    if bd2.get('status') == 'online':
                        user_bots[next_uid] = bd2
                        return jsonify({"status": "success", "message": "Random Bot চালু হয়েছে!", "uid": next_uid})
                except Exception:
                    pass
            return jsonify({"status": "error", "message": msg})
    except Exception as e:
        print(f"[random-auto] Exception: {e}")
        return jsonify({"status": "error", "message": f"সংযোগ ব্যর্থ: {str(e)[:100]}"})
