...
function handleTaskClick(taskId, link) {
    console.log("Task clicked:", taskId, "Link:", link);
...
function showUserTypeSelection() {
    ...
    document.getElementById("random-form-overlay").classList.add("hidden");
    document.getElementById("private-form-overlay").classList.add("hidden");
    document.getElementById("main-container").style.display = "none";
    document.getElementById("random-status").textContent = "";
    const rs = document.getElementById("random-status-detail");
...
function showRandomForm() {
    document.getElementById("usertype-overlay").classList.add("hidden");
    document.getElementById("random-form-overlay").classList.remove("hidden");
    const tc = document.getElementById("random-teamcode");
    if (tc) tc.value = "";
    const rs = document.getElementById("random-status-detail");
    if (rs) rs.textContent = "";
}
...
function showPrivateForm() {
    document.getElementById("usertype-overlay").classList.add("hidden");
    document.getElementById("private-form-overlay").classList.remove("hidden");
    const ls = document.getElementById("login-status");
    if (ls) ls.textContent = "";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // RANDOM USER BUTTON - সরাসরি Main UI তে নিয়ে যায়
    ...
    if (randomUserBtn) {
        randomUserBtn.addEventListener("click", function () {
            ...
            randomUserBtn.disabled = true;
            randomUserBtn.style.opacity = "0.6";
            statusDiv.innerHTML = `<span style="color:#dc143c;font-size:13px;"><i class="fas fa-spinner fa-spin"></i> Random Bot সংযুক্ত হচ্ছে... (~30s)</span>`;

            fetch('/api/random-auto', {
                method: 'POST',
    ...
    // RANDOM SUBMIT BUTTON
    const randomSubmitBtn = document.getElementById("random-submit-btn");
    if (randomSubmitBtn) {
        randomSubmitBtn.addEventListener("click", function () {
            const tc = document.getElementById("random-teamcode").value.trim();
            const statusDiv = document.getElementById("random-status-detail");

            if (!tc) {
                statusDiv.innerHTML = `<span style="color:#ff4444;">Team Code দিন!</span>`;
                return;
            }

            randomSubmitBtn.disabled = true;
            randomSubmitBtn.innerHTML = `<i class="fas fa-spinner fa-spin"></i> Connecting...`;
            statusDiv.innerHTML = `<span style="color:#dc143c;">Random Bot লগইন করছে... (~30s)</span>`;

            fetch('/api/random-submit', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ team_code: tc })
            })
            .then(r => r.json())
            .then(data => {
                randomSubmitBtn.disabled = false;
                randomSubmitBtn.innerHTML = `<i class="fas fa-robot"></i> DEPLOY BOT`;
                if (data.status === 'success' || data.status === 'already_online') {
                    statusDiv.innerHTML = `<span style="color:#00ff88;">✓ Bot চালু! UID: ${data.uid}</span>`;
                    LOGGED_IN_BOT_UID = data.uid;
                    localStorage.setItem('bot_uid', data.uid);
                    localStorage.setItem('random_tc', tc);
                    setTimeout(() => {
                        showMainUI();
                        const tcInput = document.getElementById("team-code");
                        if (tcInput) { tcInput.value = tc; saveData("tc", tc); }
                    }, 1200);
                } else {
                    statusDiv.innerHTML = `<span style="color:#ff4444;">✗ ${data.message || 'Failed'}</span>`;
                }
            })
            .catch(e => {
                randomSubmitBtn.disabled = false;
